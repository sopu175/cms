...
const Users: React.FC = () => {
  ...
  const handleEditUser = (userToEdit: UserType) => {
    ...
    setFormData({
      ...
      role: (userToEdit.role as 'admin' | 'editor' | 'author' | 'customer') || 'customer',
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {filteredUsers.map((u) => (
                <tr key={u.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 text-xs rounded-full capitalize ${getStatusColor(u.status || 'active')}`}>
                      {u.status}
                    </span>
                  </td>
                  <td className="px-6 py-4">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      value={formData.role}
                      onChange={(e) => setFormData({ ...formData, role: e.target.value as 'admin' | 'editor' | 'author' | 'customer' })}
                      className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                      <option value="customer">Customer</option>
                      <option value="author">Author</option>
