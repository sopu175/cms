...
import { Post, ContentBlock, SEOData, GalleryItem } from '../types';
...
const PostEditor: React.FC<PostEditorProps> = ({ post, onSave, onCancel }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const postData = {
      ...
      author_id: formData.author_id && formData.author_id !== '' ? formData.author_id : null,
              ...
              <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-gray-200 dark:border-gray-800">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {(formData.gallery_images as GalleryItem[]).map((item: GalleryItem, index: number) => (
                      <div key={item.id} className="relative group">
                        <img
                          src={item.type === 'image' && item.url ? item.url : ''}
                          alt={`Gallery ${index + 1}`}
                        ...
                        <button
                          ...
                          onClick={() => {
                            setFormData(prev => ({
                              ...prev,
                              gallery_images: prev.gallery_images.filter((img: GalleryItem) => img.id !== item.id)
                            }));
                          }}
            ...
            <div className="inline-block w-full max-w-4xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white dark:bg-gray-900 shadow-xl rounded-2xl">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 max-h-96 overflow-y-auto">
                {media.map((item) => (
                  ...
                  >
                    <img
                      src={typeof item === 'string' ? item : (item && 'url' in item ? item.url : '')}
