...
const Menus: React.FC = () => {
  ...
  const handleSaveMenu = () => {
    const menuData: Menu = {
      ...
      status: editingMenu?.status || 'active',
      created_at: editingMenu?.created_at || new Date().toISOString(),
      ...(editingMenu?.settings ? { settings: editingMenu.settings } : {}),
      ...(editingMenu?.updated_at ? { updated_at: editingMenu.updated_at } : {})
    };
    if (editingMenu?.id) {
      setMenus(menus.map(menu => menu.id === editingMenu.id ? menuData : menu));
    } else {
