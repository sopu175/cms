...
-- Create site_info table if it doesn't exist
...
-- Create seo_meta table if it doesn't exist
...
DO $$ 
DECLARE
  table_names text[] := ARRAY['profiles', 'categories', 'posts', 'content_pages', 'sections', 
                             'products', 'product_variations', 'orders', 'order_items', 
                             'settings', 'site_info', 'wishlists', 'reviews', 'coupons', 
                             'media', 'menus', 'forms', 'form_submissions', 'seo_meta'];
  table_name text;
  policy_name text;
  policy_names text[];
BEGIN
  ...
  LOOP
    -- Get all policies for the current table
...
-- Insert default site info if it doesn't exist
...
-- Insert sample form if it doesn't exist
